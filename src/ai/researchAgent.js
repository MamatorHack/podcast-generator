...
async function generatePodcastScript(winningSubject) {
    console.log(`\n🧠 Appel de GPT-4o pour la rédaction du script...`);
    console.log(`Sujet traité : "${winningSubject}"\n`);

    // LE MASTER PROMPT : Configuration à 4 voix
    const systemPrompt = `
    ...
    - L'animateur Mathis DOIT TOUJOURS commencer le podcast en rappelant brièvement le concept : "Une émission 100% générée par l'IA, où la communauté Reddit (sur r/testpodbot) décide du sujet de la semaine."
    ...
    - La conclusion (outro) doit remercier l'auteur du commentaire gagnant et inviter à voter pour la semaine prochaine sur r/testpodbot.
    ...
    try {
        const response = await openai.chat.completions.create({
            ...
            temperature: 0.75, // Un poil plus haut pour fluidifier les interactions à 4
